...
def drawBox2D(passed_object, count_label,idx):
	...
	blank_row = np.zeros((1,width,3), np.uint8)			
	image_ = np.concatenate((image,blank_row), axis=0)
	cv2.imwrite("image_"+"%04d.jpg"%(idx),image_)	
	cv2.imshow('figure_test_display',image_)
...
#============================================================================================
# def drawBox3D(passed_tracklets,passed_corners,passed_face_idx,passed_orientation):
# 	if ~(corners == None):
# 		for f in range(4):
...
for img_idx in range(nimages):
	...
	for obj_idx in range(count):
		...
		drawBox2D(tracklets[img_idx][obj_idx], count,img_idx)
		...
		#drawBox3D(tracklets[img_idx][obj_idx],corners,face_idx,orientation)
